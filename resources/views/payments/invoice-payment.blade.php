...
@section('title', 'دفع الفاتورة - ' . $invoice->invoice_number)
...
<section class="bg-gradient-to-r from-blue-600 to-blue-700 text-white py-12">
    <div class="container mx-auto px-4">
        <div class="text-center">
            <h1 class="text-3xl md:text-4xl font-bold mb-4">دفع الفاتورة</h1>
            <p class="text-blue-100 text-lg">إكمال عملية الدفع للفاتورة</p>
        </div>
    </div>
</section>
    ...
    <div class="container mx-auto px-4">
        <div class="max-w-4xl mx-auto">
            <div class="bg-white rounded-lg shadow-sm p-6 mb-8">
                <h2 class="text-2xl font-bold text-gray-800 mb-6">تفاصيل الفاتورة</h2>

                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        <h3 class="text-lg font-semibold text-gray-700 mb-4">معلومات الفاتورة</h3>
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-semibold">{{ number_format($invoice->original_amount) }} ر.س</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">المبلغ المدفوع:</span>
                                <span class="font-semibold text-green-600">{{ number_format($invoice->paid_amount) }} ر.س</span>
                            </div>
                            <div class="flex justify-between text-lg font-bold text-red-600">
                                <span>المبلغ المتبقي:</span>
                                <span>{{ number_format($invoice->remaining_amount) }} ر.س</span>
                            </div>
                        </div>
                    </div>

                    <div>
                        <h3 class="text-lg font-semibold text-gray-700 mb-4">معلومات الشركة</h3>
                        <div class="space-y-3">
                            @if(auth()->user()->isLogisticsCompany())
                            <div>
                                <span class="text-gray-600">الشركة الطالبة للخدمة:</span>
                                <p class="font-semibold mt-1">{{ $invoice->serviceCompany->user->company_name ?? $invoice->serviceCompany->user->name }}</p>
                            </div>
                            @else
                            <div>
                                <span class="text-gray-600">الشركة اللوجستية:</span>
                                <p class="font-semibold mt-1">{{ $invoice->logisticsCompany->user->company_name ?? $invoice->logisticsCompany->user->name }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <form action="{{ route('payments.store') }}" method="POST" id="payment-form">
                    ...
                    <input type="hidden" name="related_id" value="{{ $invoice->id }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    accountOptions.forEach(option => {
        option.addEventListener('click', function() {
            ...
            accountOptions.forEach(o => o.classList.remove('border-blue-500', 'bg-blue-50'));
